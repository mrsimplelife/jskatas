...
describe("initialize a `Map`", function () {
  ...
  it("init Map with multiple key+value pairs", function () {
    const pair1 = [1, "one"] as const;
    const pair2 = [2, "two"] as const;
  ...
  it("keys are unique, the last one is used", function () {
    const pair1 = [1, "one"] as const;
    const pair2 = [1, "uno"] as const;
    const pair3 = [1, "eins"] as const;
    const pair4 = [2, "two"] as const;
  ...
  it("init Map from an Object, is a bit of work", function () {
    ...
    keys.forEach((key) => map.set(key, obj[key as keyof typeof obj]));
...
function mapToArray(map: Map<any, any> | Array<any>) {
  return Array.from(map);
}
function assertMapsEqual(
  map1: Map<any, any>,
  map2: Map<any, any> | Array<any>
) {
