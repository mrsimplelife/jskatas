...
describe("chaining multiple promises can enhance readability", () => {
  ...
  describe("chain promises", function () {
    const removeMultipleSpaces = (s: string) => s.replace(/\s+/g, " ");
    ...
    const asyncUpperCaseStart = (
      s: string,
      onDone: (value: string | PromiseLike<string>) => void
    ) => {
    ...
    it("any of the things given to `then()` can resolve asynchronously (the real power of Promises)", function () {
      ...
      return wordsPromise
        .then(
          (s) =>
            new Promise<string>((resolve) => asyncUpperCaseStart(s, resolve))
        )
        .then(
          (s) =>
            new Promise((resolve) =>
